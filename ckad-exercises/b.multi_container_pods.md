...
### Ex-B02
Create a pod with an `nginx` container exposed on `port 80`. Add a `busybox init container` which downloads a page using `"wget -O /work-dir/index.html http://neverssl.com/online"`. Make a `volume of type emptyDir` and mount it in both containers. 
