...
### Ex-G02
Create a `PersistentVolume` of `10Gi`, called `myvolume`. Make it have `accessMode` of `ReadWriteOnce` and `ReadWriteMany`, storageClassName `normal`, mounted on hostPath `/etc/foo`. 
...
Create it on the cluster. Show the PersistentVolumeClaims of the cluster. 
...
> 如果是多节点主机需要确认busybox部署的节点，才能找到对应的本地hostpath路径。
...
> 可以通过pod affinity使两个pod部署到同一个节点上
